...
CREATE TABLE roles (
    ...
    name VARCHAR(50) UNIQUE NOT NULL, -- 'admin', 'general'
    description TEXT,
...
('general', 'Regular User - access to own resources');
...
CREATE TABLE api_users (
    ...
    role_id UUID REFERENCES roles(id), -- Foreign Key to roles
...
CREATE TABLE whatsapp_sessions (
    ...
    session_id VARCHAR(255) NOT NULL, -- The session identifier string (e.g. 'marketing')
    status VARCHAR(50) DEFAULT 'DISCONNECTED', -- 'CONNECTED', 'SCANNING', 'DISCONNECTED'
    auth_data JSONB, 
    api_key VARCHAR(255), -- API Key specific to this session
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(user_id, session_id), -- User can't have duplicate session names
...
CREATE TABLE webhooks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id VARCHAR(255) NOT NULL, -- Logical session ID (name)
    url TEXT NOT NULL,
    events TEXT[], -- Array of event strings
